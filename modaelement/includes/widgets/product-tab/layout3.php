...
echo '<!-- popular product section start  -->
    <section class="moda-product-popular-section moda-section pt-0">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <div class="moda-title-heading moda-title-heading-width">
                        <h3 >' . $settings['title'] . '</h3>
                    </div>
                </div>
                <div class="col-lg-6">
                    <nav class="moda-tablist moda-tablist-secound">
                        <div class="nav nav-tabs" id="nav-tab-one" role="tablist">';
$loop = 0;
foreach ($categories as $category) {
    ...
    echo '<button class="' . $act . '" id="nav-' . $category->slug . '-tab-one" data-bs-toggle="tab" data-bs-target="#nav-' . $category->slug . '-one" type="button" role="tab" >' . $category->name . ' </button>';
}
echo '</div>
                    </nav>
                </div>
            ...
            <div class="row moda-product-popular" >
                ...
                <div class="col-lg-6 p-0">
                    <div class="tab-content " id="nav-tabContent-one">';
$loop2 = 0;
foreach ($categories as $category) {
    $loop2++;
    if ($loop2 == 1) {
...
echo '</div>
                ...
                <div class="col-lg-3 pr-0 pl-md-0">
                    <div class="moda-categories-wedgets">
                        <div class="heading">
                            <h6>Categories</h6>
                        </div>
                        <ul>';
foreach ($categories2 as $cat) {
    echo '<li><a href="' . get_term_link($cat->slug, $cat->taxonomy) . '"><span><i class="far fa-angle-right"></i></span>' . $cat->name . ' </a></li>';
}
echo '</ul>
                    </div>
                </div>
