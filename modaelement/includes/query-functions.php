...
{
    return array(
        'none' => __('No order', 'modaelement'),
        'ID' => __('Post ID', 'modaelement'),
        'author' => __('Author', 'modaelement'),
        'title' => __('Title', 'modaelement'),
        'date' => __('Published date', 'modaelement'),
        'modified' => __('Modified date', 'modaelement'),
        'parent' => __('By parent', 'modaelement'),
        'rand' => __('Random order', 'modaelement'),
        'comment_count' => __('Comment count', 'modaelement'),
        'menu_order' => __('Menu order', 'modaelement'),
        'post__in' => __('By include order', 'modaelement'),
    );
}

//
/* First letter of Content */
/*function moda_first_letter() {
...
function moda_social_post_share()
{
...
function moda_post_title()
{
    global $post;
    $id = $post->ID;
    // $fletter = moda_first_letter();
...
function madmag_ft_images($thumbnail)
{
    ...
    $placeholder = Moda_PLUGIN_URL . 'assets/lazyload.svg';
...
function moda_post_title_nolink()
{
    global $post;
    $id = $post->ID;
    //$fletter = moda_first_letter();
...
function moda_excerpt_shortcode($limit)
{
    if ($limit > "0") {
        global $post;
        $id = $post->ID;
...
function moda_excerpt_folio($limit)
{
    if ($limit > "0") {
        global $post;
        $id = $post->ID;
    ...
    if (is_array($metas)) {
        ...
        foreach ($metas as $meta) {
            ...
            switch ($meta) {
                ...
                case 'excerpt':
                    $out = moda_excerpt_shortcode($excerpt_length);
                    break;
...
function moda_get_category_link($taxonomy)
{
...
function misha_loadmore_ajax_handler()
{
    ...
    require Moda_PLUGIN_DIR . 'includes/loops/' . $template . '.php';
    die();
}
...
function misha_tabcat_ajax_handler()
{
    ...
    //require(Moda_PLUGIN_DIR.'includes/loops/tabcat.php');
    require Moda_PLUGIN_DIR . 'includes/loops/' . $template . '.php';
    die();
}
